...
const Navbar = () => {
  ...
  return (
    <div className='fixed top-0 left-0 z-50 w-full flex items-center justify-between px-6 md:px-16 lg:px-3 py-5'>
        ...
        !user?(<button onClick={() => openSignIn({})} className='px-4 py-1 sm:px-7 sm:py-2 bg-primary hover:bg-primary-dull transition rounded-full font-medium cursor-pointer'>Login</button>):(
          <UserButton>
            <UserButton.MenuItems>
              <UserButton.Action label='My-Bookings' labelIcon={<TicketPlus width={15}/>} onClick={()=>nevigate('/my-bookings')}/>
            </UserButton.MenuItems>
          </UserButton>
        )
      }
